...
* A traversal namespace ``++addtile++``, and an associated form, which can be
  used to create a new tile based on the tile's schema. For transient tiles,
  this merely redirects to a URL with an appropriate query string. For
  persistent tiles, it will also save the necessary data. This will fire an
  ``IObjectCreatedEvent`` for the newly created tile (a transient object)
  when successfully submitted.

* A traversal namespace ``++edittile++``, and an associated form, which can
  be used to edit a tile based on the tile's schema. This will fire an
  ``IObjectModifiedEvent`` for the modified tile (a transient object)
  when successfully submitted.
...
* A view ``@@available-tiles``, which allows the user to choose from all
  available tiles (subject to the tile's add permission) and redirects to the
  appropriate ``++addtile++`` URL to configure the tile.
